...
double evaluate_quality(py::array_t<int> from_arr, py::array_t<int> to_arr, py::array_t<double> w_arr, int n_edges, py::array_t<double> null_model_input_arr, int num_null_vectors, double time, py::array_t<int> partition_arr) {
    ...
    partition start_partition(part_test);//lemon::countNodes(input_graph));

    clq::find_linearised_generic_stability quality(current_markov_time);
    
    double stability = clq::find_stability<partition>(
                           input_graph, input_graph_weights, null_model, quality,
                           start_partition, 1e-18);
...
PYBIND11_MODULE(generalizedLouvain_API, m) {
    m.doc() = R"pbdoc(
        Pybind11 binding of Generalized Louvain
        -----------------------------------------

        .. currentmodule:: generalizedLouvain_API 
